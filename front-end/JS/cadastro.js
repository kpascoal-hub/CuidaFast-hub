...
<script type="module">
  ...
  document.addEventListener("DOMContentLoaded", () => {
      ...
      btnGoogle.addEventListener("click", () => {
        signInWithPopup(auth, provider)
          .then((result) => {
            ...
            // Redirecionamento com base na escolha do usuário
            if (btnCuidador && btnCuidador.classList.contains("active")) {
              window.location.href = "CadastroTipoProfissional.html";
            } else if (btnCliente && btnCliente.classList.contains("active")) {
              window.location.href = "HomeCliente.html";
            } else {
              alert("Selecione o tipo de usuário antes de continuar.");
            }
          })
          .catch((error) => {
            console.error("Erro no login com Google:", error);
    ...
    // Alternância entre cuidador e cliente
    if (btnCuidador && btnCliente) {
      btnCuidador.addEventListener("click", () => {
        btnCuidador.classList.add("active");
        btnCuidador.classList.remove("inactive");
        btnCliente.classList.remove("active");
    ...
    // Login com formulário (email e senha)
    if (form) {
      form.addEventListener("submit", (event) => {
        event.preventDefault();

        const isCuidador = btnCuidador && btnCuidador.classList.contains("active");
        const isCliente = btnCliente && btnCliente.classList.contains("active");
        ...
        // Aqui você pode autenticar via backend ou Firebase
        // Exemplo de redirecionamento simulado:
        if (isCuidador) {
          window.location.href = "CadastroTipoProfissional.html";
        } else if (isCliente) {
          window.location.href = "HomeCliente.html";
